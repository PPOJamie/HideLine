...
function deductionCoordinateFields(prefix, label, current = null) {
  ...
  return `
    <fieldset class="coordinate-pair compact-coordinate-pair simple-coordinate-pair">
      ...
      <div class="coordinate-selection-summary ${lat && lng ? "selected" : ""}" data-coordinate-summary="${prefix}">${icon("location")}<span>${lat && lng ? `${lat}, ${lng}` : "No point selected yet"}</span></div>
