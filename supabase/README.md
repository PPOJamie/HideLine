...
Existing `team_states` rows do not need a table migration because their `state` column is JSONB. The 1.1 client adds the property when a team first saves its deduction board.
...
- private station, card, note and deduction-map state is visible only to the same team;
...
Map-ready questions may include the seeker pin, travel endpoints or line/stops in shared question history. This is information the hiders ordinarily need to answer the question. The station elimination results, manual constraints, ignored-answer list and priority marks remain in the seeker team's private JSON state.
