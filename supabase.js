...
import { questionLocations, questionLocationSummary } from "./core/question-location.js";
import {
...
class HideLineApp {
  ...
  renderNotifications() {
    const region = document.getElementById("game-alert-region");
    if (!region) return;
  ...
  alertPendingQuestion(record) {
    if (!record?.id || this.alertedQuestionIds.has(record.id)) return;
  ...
  hydrateRemote(hydrated) {
    ...
    queueMicrotask(() => {
      const pending = this.store.get().questions.filter((question) => question.status === "pending");
      pending.forEach((question) => this.alertPendingQuestion(question));
    });
  }
  ...
  async askQuestion(questionId, data, form) {
    ...
    await this.recordEvent("question", { questionId, questionName: definition.name, prompt, note: record.note, occurrence, reward, questionInstanceId: record.id, pinLabel: record.pinLabel, locations, locationSummary: questionLocationSummary(record), round });
    this.closeModal();
