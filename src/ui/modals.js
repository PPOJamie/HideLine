...
function questionDeductionFields(state, question) {
  ...
  if (config.mode === "guided") {
    ...
    fields = `${deductionCoordinateFields("deductionSeeker", "Seeker pin for Thames-side matching", current)}<div class="callout">${icon("info")}<p>HideLine detects north, south or the bridge/tunnel river corridor from this pin automatically.</p></div>${deductionMovementSelect(state)}`;
